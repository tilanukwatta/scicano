...
This program handles arxiv papers published starting from April 2007 to December 2014.
...
def show_page(data, url):
    ...
    abstract = soup.text.encode('utf-8').decode('ascii', 'ignore')[soup.text.find("Abstract:")+9:soup.text.find("CONTEXTComments:")]
...
if __name__ == '__main__':
    ...
    for yy in year:
        for mm in month:
            ar_end = 0
            for ar in article:
                url = preURL + yy + mm + '.' + str(ar).zfill(4)
                #url_id = url[-7:]  # this for articles on or before 1999
                url_id = '20' + url[-9:]  # this for articles on or after 2000
